...
from models.Neurons import PVBMLIF,LIF,PVBMLIFH
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride, modified=True):
        ...
        # self.spike_func = LIF()
        self.spike_func = PVBMLIF(max_threshold=1)
        # self.spike_func2 = LIF()
    ...
    def forward(self, x):
        # out,tmp_time = self.spike_func(self.bn1(self.conv1(x)))
        out = self.spike_func(self.bn1(self.conv1(x)))

        out = self.bn2(self.conv2(out))
        if self.modified:
            out = self.spike_func(out)
            # out_tmp, time = self.spike_func2(out)
            # out = out_tmp + self.shortcut(x)
            out += self.shortcut(x)  # Equivalent to union of all spikes  先转换为脉冲再相加，即DS-ResNet
        else:
            out += self.shortcut(x)
            # out,tmp_time = self.spike_func2(out)
            out = self.spike_func(out)
        return out
