...
def main():
    ...
    * :ref:`中文API <lif_fc_mnist.main-cn>`
    ...
    parser.add_argument('-data-dir', type=str, default='/home/hp/DataSet/MNIST', help='root dir of MNIST dataset')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for img, label in tqdm(train_data_loader):
            ...
            if scaler is not None:
                with amp.autocast():
                    encoded_img = encoder(img)  # [b,1,28,28]
                    data_temp = encoded_img.to(args.device)
                    bs = data_temp.shape[0]
                    input_data = torch.zeros((args.T * bs,) + data_temp.shape[1:], device=data_temp.device)
                    for t in range(args.T):
                        input_data[t * bs:(t + 1) * bs, ...] = data_temp
                    out_fr = net(input_data)
