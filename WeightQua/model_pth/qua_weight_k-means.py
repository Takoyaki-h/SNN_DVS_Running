...
def find_scale_factor(tensor, num_bits):
    ...
    max_val = np.max(tensor)  # 获取张量的最大值
    # scale_factor 调整为适用于无符号量化。无符号4-bit的范围是0到15
    scale_factor = (2 ** num_bits - 1) / max_val  # 计算缩放因子
    return scale_factor


# 基于聚类的方法进行权值量化，量化为4-bit无符号数
def k_means_cpu(weight, n_clusters, init='k-means++', quantization_bits=4):
    ...
    weight = torch.round(weight * scale_factor).clamp(0, 2 ** quantization_bits - 1)
...
for name, param in model.named_parameters():
    if 'weight' in name:
        param.data = k_means_cpu(param.data, n_clusters=16, quantization_bits=4)[2]
